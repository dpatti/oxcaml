...
(* Emit: if (__sancov_should_track) __sanitizer_cov_trace_pc_guard(&guards[n])
   where each guard is a uint32 (4 bytes).

   [__sancov_should_track] is the int64 gate that LLVM's
   [-sanitizer-coverage-gated-trace-callbacks] option emits and tests; it is
   defined (weak, hidden, zero-initialised) by whatever provides the callbacks
   at link time, and set by the runtime to enable tracing. Checking it inline
   keeps the cost of instrumented code down to a well-predicted branch when
   coverage collection is not active (e.g. in build tools, or when running
   outside the coverage environment), which an unconditional call cannot
   achieve even with an empty callee because of the register spills the call
   forces. Unlike clang we reload the gate at every edge rather than hoisting
   it to the function entry, so as not to pin a register. *)
  ...
  let enabled =
    Cop
      ( Cload
        ...
        [Cconst_symbol (global_sym "__sancov_should_track", dbg)],
        dbg )
  in
  Cifthenelse
