...
(** Coverage instrumentation compatible with LLVM's SanitizerCoverage
    ...
    Each branch-target call is guarded by a test of the int64 global
    [__sancov_should_track], as with clang's
    [-sanitizer-coverage-gated-trace-callbacks]; the link must define
    it (clang emits a weak, hidden, zero-initialised definition in
    every instrumented module).  This lets the provider of the
    callbacks turn the instrumentation into a predictable branch when
